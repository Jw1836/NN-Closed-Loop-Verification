...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.hidden1 = torch.nn.Linear(
            1, 5
        )  # 1 neuron in input layer, 5 neurons in 1st hidden layer 1
...
nnc = Net()
nnc.load_state_dict(
    torch.load("/Users/jwayment/Code/NN-Closed-Loop-Verification/my_model.pth")
)
...
V_vals = net(x_plot)
dV_dx = torch.autograd.grad(V_vals, x_plot, torch.ones_like(V_vals), create_graph=True)[
    0
]
...
plt.plot(
    x_plot.detach().numpy(), dV_dt_vals, label="Lie derivative dV/dt", linestyle="--"
)
