...
def find_beta_max_min(w, x, theta, delta):
    ...
    # Constraints
    constraints = [beta == w @ (x + dx) + theta, cp.norm_inf(dx) <= delta]
...
def find_max_sensitivity(x_val, delta):
    ...
    for i in range(L - 1):
        ...
        for j in range(neurons):
            ...
            gamma = find_max_gamma(
                beta_max, beta_min, x_val, w[j, :], theta[j], function
            )
...
# θ^[1]
theta1 = np.array([[1.1836], [-0.9087], [-0.3463], [0.2626], [-0.6768]])
...
for i in range(len(x_1_centers)):
    for j in range(len(x_2_centers)):
        ...
        rect = Rectangle(
            (output[0] - max_sen, output[1] - max_sen),
            2 * max_sen,
            2 * max_sen,
            linewidth=1,
            edgecolor="r",
