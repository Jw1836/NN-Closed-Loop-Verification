...
class dataPlotter:
    def __init__(self):
        ...
        self.handle = []
        self.handle.append(
            myPlot(self.ax, xlabel="time", ylabel="x_pos", title="Dot Data")
        )
        # self.handle.append(myPlot(self.ax[1], xlabel='t(s)', ylabel='torqe(N-m)'))

    def update(self, t: float, states: np.ndarray, ctrl: float, reference: float = 0.0):
        ...
        self.handle[0].update(
            self.time_history, [self.theta_history, self.theta_ref_history]
        )
