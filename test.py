...
def find_equilibrium_numeric(r, x0=None, maxit=50, tol=1e-10):
    ...
    for _ in range(maxit):
        ...
        h = 1e-6
        fpx = (f_closed_loop_scalar(x + h, r) - f_closed_loop_scalar(x - h, r)) / (
            2 * h
        )
        if abs(fpx) < 1e-8:
            break
...
def verify_local_lyapunov(r, neigh=1.0, domain_points=201, verbose=True):
    ...
    if verbose:
        ...
        if is_pos_def and is_neg_def:
            ...
            if not is_pos_def:
                print("   - V not positive definite (check domain).")
            if not is_neg_def:
                print("   - dV not negative definite (try smaller neighborhood).")
