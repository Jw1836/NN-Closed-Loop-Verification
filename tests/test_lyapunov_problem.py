...
class TestCheckOrigin:
    def test_passes_when_v0_is_zero(self):
        ...
        assert np.isclose(result["v0"], 0.0)
        assert result["counterexamples"] is None
    ...
    def test_fails_when_v0_is_nonzero(self):
        ...
        cexs = result["counterexamples"]
        assert cexs is not None and len(cexs) == 1
        # First state_dim entries are the origin
        assert np.allclose(cexs[0][:-1], 0.0)
        # Last entry is V(0)
        assert np.isclose(cexs[0][-1], 1.0)

    def test_negative_v0_is_also_a_violation(self):
        prob = _make_problem(ConstantNet(-0.5))
        result = prob.check_origin()
        assert not result["passed"]
        assert np.isclose(result["v0"], -0.5)
...
class TestCheckPositive:
    ...
    def test_detects_violation_when_v_nonpositive(self):
        ...
        assert not result["passed"]
        cexs = result["counterexamples"]
        assert cexs is not None and len(cexs) > 0
        assert len(cexs[0]) == prob.state_dim + 1
        # All returned V values should be ≤ 0
        assert all(row[-1] <= 0.0 for row in cexs)
    ...
    def test_violation_rows_contain_correct_coordinates(self):
        ...
        assert all(np.isclose(row[-1], -1.0) for row in result["counterexamples"])

    def test_skips_vertices_within_hole(self):
        # If all intersections are inside the hole radius, passed is True.
